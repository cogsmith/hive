...
App.LoadCell = function (cell) {
	...
	if (z.Type == 'APPJS') { if (!fs.existsSync(cellpath + '/package.json')) { donpmi = "echo '" + celltag + '=APPJS_NOPACKAGE=' + cell + "'"; LOG.INFO('APPJS_NOPACKAGE'); } }
	if (z.Type == 'APPJS') { RUN.push("cd " + cellpath + " ; docker stop " + dockid + " ; docker wait " + dockid + " ; sleep 1 ; docker rm -f " + dockid + " ; " + donpmi + " ; docker run -d -t --restart always --name " + dockid + ' --env HIVESLUG=' + slug + ' --env SLUGHOST=' + slughost.toLowerCase() + " --env HOST=0.0.0.0 --env PORT=9 --env CELLTAG=" + celltag + " -p " + App.HiveBind + ":" + z.Port + ":9 -v " + z.Path + ":/app -v /hive/WX/node_modules:/node_modules cogsmith/nodemon nodemon --delay 2.5 --ignore package.json --ignore package-lock.json /app/app.js --port 9 --ip 0.0.0.0 --loglevel trace"); }
