...
App.LoadCell = function (cell) {
	...
	//if (z.Type == 'APPJS') { if (!fs.existsSync(z.Path + '/package.json')) { fs.writeFileSync(z.Path + '/package.json', JSON.stringify({ dependencies: { '@cogsmith/xt': '*' } })); } } // ENOENT ?
	if (z.Type == 'APPJS') { if (!fs.existsSync(z.Path + '/package.json')) { RUN.push("echo '{\"dependencies\":{\"@cogsmith/xt\":\"*\"}}' > package.json"); } }
