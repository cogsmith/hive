...
App.LoadCell = function (cell) {
	...
	if (z.Type == 'APPJS') { RUN.push("docker stop " + dockid + " ; docker rm " + dockid + " ; docker run --rm --name " + dockid + "_NPM -v " + z.Path + ":/app cogsmith/nodemon npm remove @cogsmith/xt ; docker run --rm --name " + dockid + "_NPM -v " + z.Path + ":/app cogsmith/nodemon npm ci ; docker run --restart always --name " + dockid + ' --env HIVESLUG=' + slug + ' --env SLUGHOST=' + slughost.toLowerCase() + " --env HOST=0.0.0.0 --env PORT=9 -p " + App.HiveBind + ":" + z.Port + ":9 -v " + z.Path + ":/app cogsmith/nodemon nodemon /app/app.js --port 9 --ip 0.0.0.0"); }
