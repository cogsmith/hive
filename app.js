...
App.LoadCell = function (cell) {
	...
	if (z.Type == 'HTML') { runrun = "docker stop " + dockid + " ; docker rm " + dockid + " ; docker run --rm --name " + dockid + ' --env HIVESLUG=' + slug + ' --env SLUGHOST=' + slughost.toLowerCase() + " --env HOST=0.0.0.0 --env PORT=9 -p " + App.HiveBind + ":" + z.Port + ":9 -v " + z.Path + ":/www cogsmith/wx-static --port 9 --ip 0.0.0.0 --www /www --base /"; if (!cz[1] && cz[1] != '@') { cz.join('/').replace(slug + '/web/raw/', '') + '/' }; RUN.push(runrun); }
